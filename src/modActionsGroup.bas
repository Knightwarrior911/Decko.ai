...
Public Sub Do_group_shapes(slideNum As Long, shapeIds As Variant, _
                            Optional refName As String = "")
    Dim ids() As Long
    Dim cnt As Long: cnt = modActionsLayout.NormalizeIdsArray(shapeIds, ids, slideNum)
    ...
    For i = 0 To cnt - 1
        ...
        names(i) = sh.Name
    Next i
    Dim rng As ShapeRange: Set rng = sl.Shapes.Range(names)
    Dim grp As Shape: Set grp = rng.Group()
    If Len(refName) > 0 Then grp.Name = refName
End Sub
