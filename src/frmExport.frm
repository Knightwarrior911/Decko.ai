...
Private Function PromptTemplate() As String
    ...
    s = s & "     (the slide currently mentioning the company they want to replace," & vbCrLf
    s = s & "     the slide showing the chart they want changed, etc.)." & vbCrLf
    s = s & "   - Slide 1 is only the right answer when the snapshot has exactly one" & vbCrLf
    s = s & "     slide OR when slide 1 is demonstrably the target. Otherwise, slide 1" & vbCrLf
    s = s & "     is no more likely than any other slide." & vbCrLf
