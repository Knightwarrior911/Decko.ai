...
> `money`, `arrow_trending`, `shield`, `vehicle_car`). The prompt ships a
> short curated list + CDN-sourcing guidance (no exhaustive allow-list):
> prefer a curated name, otherwise pick the closest semantic Fluent name.
...
## 17. High-level authoring: templates, decks-as-code, Deck DNA

Use these when the request is "make a slide/deck", not "tweak this box".
One action replaces a wall of atomic placement ops.

**The 7 built-in templates and their exact required slots:**

| `template` | required `content` slots |
|---|---|
| `title` | `title`, `subtitle` |
| `section` | `section_number`, `section_title` |
| `bullets` | `heading`, `bullets` (array of strings) |
| `two_col` | `heading`, `left_body`, `right_body` |
| `comparison` | `heading`, `left_label`, `left_body`, `right_label`, `right_body` |
| `kpi_dashboard` | `heading`, `tiles` (array of `{stat,label}`) |
| `quote` | `quote_text`, `attribution` |

Slot names are literal. A missing required slot skips the action. There is
no `two_column` / `image_caption` — use the names above exactly.

**Built-in template — section divider**

*VP prompt:* `Add a section divider: "01" / "Q3 Results".`
*Actions:*
```json
{"actions":[
  {"type":"apply_template","template":"section","content":{"section_number":"01","section_title":"Q3 Results"}}
]}
```

**Built-in template — replace slide 3 with a two-column slide**

*VP prompt:* `Turn slide 3 into a two-column slide: left = Plan, right = Actual.`
*Actions:*
```json
{"actions":[
  {"type":"apply_template","template":"two_col","slide":3,"content":{"heading":"Plan vs Actual","left_body":"Launch in EU\nHire 12 reps\n$4M pipeline","right_body":"Launched EU + UK\nHired 9 reps\n$5.1M pipeline"}}
]}
```

**Built-in template — KPI dashboard**

*VP prompt:* `A KPI row: 8.4M DAU, $42 ARPU, NPS 61.`
*Actions:*
```json
{"actions":[
  {"type":"apply_template","template":"kpi_dashboard","content":{"heading":"Engagement Snapshot","tiles":[{"stat":"8.4M","label":"Daily Active Users"},{"stat":"$42","label":"ARPU"},{"stat":"61","label":"NPS"}]}}
]}
```

**Decks-as-code — build a 3-slide deck**

*VP prompt:* `Build a 3-slide deck: title, a bullets slide of 3 wins, a closing CEO quote.`
*Actions:*
```json
{"actions":[
  {"type":"build_deck_from_spec","spec":[
    {"template":"title","content":{"title":"FY24 Review","subtitle":"Board Briefing"}},
    {"template":"bullets","content":{"heading":"Key Wins","bullets":["Revenue +22%","NPS 61 (+7)","Churn down to 4.1%"]}},
    {"template":"quote","content":{"quote_text":"Our best year on every metric that matters.","attribution":"CEO, FY24 town hall"}}
  ]}
]}
```
*(Add `"clear_existing":true` to replace the deck instead of appending.)*

**Decks-as-code — extract the deck back out as a spec**

*VP prompt:* `Give me this deck as a spec so I can restyle it.`
*Actions:*
```json
{"actions":[
  {"type":"extract_spec"}
...
**Variants — distinct layout archetypes of the same content**

`generate_variants` ignores the layout name structurally: it pulls the
heading from whichever of `title` / `heading` / `section_title` /
`quote_text` is present and folds the remaining values into the body, then
renders each as a different archetype (Hero → Split → Stack → Quote →
Tiles, cycling).

*VP prompt:* `Show me three different layouts for the DAU headline.`
*Actions:*
```json
{"actions":[
  {"type":"generate_variants","template":"hero","n":3,"content":{"heading":"8.4M Daily Active Users","detail":"+23% YoY","detail2":"71% mobile share"}}
]}
```
*(Or render the SAME content across specific named templates so the user
picks: `{"type":"generate_variants","templates":["title","bullets","quote"],"content":{...}}` — content must satisfy every listed template's slots.)*

**Deck DNA — capture, list, rename, delete a user template**

*VP prompt:* `Save slide 5 as a template called "kpi_card", then show my templates.`
*Actions:*
```json
{"actions":[
  {"type":"capture_template","name":"kpi_card","slide":5},
  {"type":"list_templates"}
]}
```

*VP prompt:* `Rename "kpi_card" to "metric_tile", then delete the old "draft_layout".`
*Actions:*
```json
{"actions":[
  {"type":"rename_template","from":"kpi_card","to":"metric_tile"},
  {"type":"delete_template","name":"draft_layout"}
]}
```
Note `rename_template` uses **`from`** and **`to`** (not `name`/`new_name`).
`capture_template` requires `name`; `slide` is optional (default active).

Once captured, the name is usable like a builtin in `apply_template` /
`build_deck_from_spec`. The captured-template manifest at the end of the
snapshot prompt lists each captured template's slots — read it instead of
guessing slot names.
