...
- **`set_chart_axis`** — fine axis control. `req:` `slide`, `shape_id`, `axis`(`x`|`y`|`y2`|`x2`), `props`(object — any of: `min`(num), `max`(num), `major_unit`(num), `title`(string), `number_format`(string), `tick_label_position`(`low`|`high`|`next_to_axis`|`none`), `scale_type`(`linear`|`logarithmic`), `major_tick_mark`(`outside`|`inside`|`cross`|`none`)).
- **`set_chart_format`** — chart-group props. `req:` `slide`, `shape_id`, `props`(object — any of: `gap_width`(0–500), `overlap`(-100–100), `bar_shape`(`box`|`cone`|`cone_to_max`|`cylinder`|`pyramid`|`pyramid_to_max`), `vary_by_categories`(bool), `reverse_categories`(bool), `reverse_series`(bool), `scale_type`(`linear`|`logarithmic`), `doughnut_hole_size`(10–90)).
- **`set_chart_series`** — per-series props. `req:` `slide`, `shape_id`, `series_index`, `props`(object — any of: `chart_type`(string, for combo charts), `axis_group`(`primary`|`secondary`), `marker_style`(`circle`|`square`|`triangle`|`diamond`|`x`|`none`), `line_dash`(`solid`|`dash`|`dot`|`round_dot`|`dash_dot`|`long_dash`|`long_dash_dot`), `fill_color`(`#RRGGBB`), `pattern`(`dotted_5`…`zig_zag` — see Office pattern names), `gradient_direction`(`horizontal`|`vertical`|`diagonal_up`|`diagonal_down`|`from_corner`|`from_center`), `data_labels`(bool), `data_label_position`(`outside_end`/`above`|`inside_end`|`inside_base`|`center`|`below`|`left`|`right`)).
